...
path <- args[1]
#path <- "C://Users//�����//Documents//Course//git//blockchain-benchmarking//post-processing//src//main//resources//results//current scenario//csv"
setwd(path)

intensities <- read.csv(file = "intensity.csv", sep = ",", head = TRUE)
blocks <- read.csv(file = "blocks.csv", sep = ",", head = TRUE)
mediumTimes <- read.csv(file = "mediumTimes.csv", sep = ",", head = TRUE)
unverified <- read.csv(file = "timeToUnverifiedTransactions.csv", sep = ",", head = TRUE)
...
distr95 <- data.frame(mediumTimes["time"], mediumTimes["distributionTime95"])
distr100 <- data.frame(mediumTimes["time"], mediumTimes["maxDistributionTime"])
bmp(filename = paste("distribution.bmp"))
plot(distr95, col = c("red"), pch = 16)
points(distr95, col = c("green"), pch = 10)
dev.off()

verification <- data.frame(mediumTimes["time"], mediumTimes["verificationTime"])
bmp(filename = "verificationTime.bmp")
plot(verification)
dev.off()
