...
nohup $GETH --mine --etherbase=$(shuf -n 1 ~/common_init_files/root_init_result/accounts) --bootnodes $(cat ~/common_init_files/root_init_result//enode) >/dev/null 2>ethereum.log &